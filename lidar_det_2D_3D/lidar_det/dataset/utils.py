...
def collate_sparse_tensors(batch):
    ...
    # C_batch = np.concatenate((batch_inds, C_batch), axis=1) # (batch, x, y, z) for torchsparse > 2.0
    C_batch = np.concatenate((C_batch, batch_inds), axis=1) # (x, y, z, batch)
    # print(C_batch[0][:], C_batch[-1][:] )
