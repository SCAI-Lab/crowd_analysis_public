...
class Trainer(object):
    def __init__(self, logger, optimizer, cfg):
        ...
        self.labels_kitti_path = cfg["path_kitti_labels"]
    ...
    def evaluate(
        ...
        tb_dict = model.model_eval_collate_fn(
            ...
            full_eval=full_eval,
            rm_files=rm_files,
            labels_kitti_path = self.labels_kitti_path,
            visible_2D=visible_2D
        )
        prec_list = []
        rec_list = []
        ...
        for k, v in tb_dict.items():
            ...
            elif k[:3] == 'ALL' and k.find('Precision') != -1 and plotting:
                # print(k)
                # print(k.find('Precision'))
                prec = tb_dict[k]
                rec_key = k.replace('Precision', 'Recall', 1)
                rec = tb_dict[rec_key]
                conf_key = k.replace('Precision', 'Confidences', 1)
                confidences = tb_dict[conf_key]
                label = k.replace('Precision','').replace('/',"_")
                prec_list.append(prec)
                rec_list.append(rec)
                confidences_list.append(confidences)
                model.plot_p_r_curves(prec, rec, confidences, self._logger.get_save_dir(self._epoch, split), label)
        model.plot_multiple_p_r_curves(prec_list, rec_list, confidences_list, tb_dict['MAX_3D_DISTANCES'], self._logger.get_save_dir(self._epoch, split), 'P-R Curves for different distances')
    ...
    def _evaluate_batch(self, model, batch, full_eval, plotting, output_dir, raise_oom, visible_2D=False,):
        try:
