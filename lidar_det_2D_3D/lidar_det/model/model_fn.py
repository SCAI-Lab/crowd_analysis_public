...
from lidar_det.utils.viz_plt import plot_p_r, plot_multiple_p_r
...
def model_fn(model, batch_dict, target_mode, disentangled_loss, reduce_batch=False):
    ...
    if isinstance(net_pred, tuple):
        ...
        # # --------
        # print(" I DO GO HERE ")
        # voxel_center = net_input.C[:, 1:].clone().float() + 0.5 # Batch went to first index  for torchsparse > 2.0
        voxel_center = net_input.C[:, :3].clone().float() + 0.5 # Batch last index
        # print("VOXEL CENTER IS" +  str(voxel_center))
    ...
    # confidences = torch.sigmoid(cls_pred)
    # print(confidences[:100,:,:], torch.max(confidences))
...
def model_eval_fn(
    model, batch_dict, full_eval=False, plotting=False, output_dir=None, nuscenes=False, visible_2D=False,
):
    ...
    # batch_inds = batch_dict["net_input"].C[:, 0] # Batch changed to first coordinate for torchsparse > 2.0
    batch_inds = batch_dict["net_input"].C[:, 3] # Batch is last coordinate
    if inds_map is not None:
...
def write_jrdb_results(boxes, scores, score_thresh, batch_dict, i_batch, output_dir, plotting, lidar_type='3D'):
    # store detection results
    ...
    if lidar_type == '2D' and "anns_valid_mask" in batch_dict:
        annotations_mask_dir = os.path.join(output_dir, f"annotations_mask_2D/{sequence}")
        # Ensure the directory exists
        os.makedirs(annotations_mask_dir, exist_ok=True)
        # Construct the full file path
        file_path = os.path.join(annotations_mask_dir, f"{frame_id}.npy")
        # Save the numpy array to the file
        np.save(file_path, batch_dict["anns_valid_mask"][i_batch])
    ...
    if plotting:
        ...
        if lidar_type == '3D':
            ...
            fig, ax = plot_bev(
                laser_data=batch_dict["laser_data"][i_batch][-1],
                boxes=boxes,
                scores=scores,
                score_thresh=score_thresh,
                boxes_gt=boxes_gt,
...
def plot_p_r_curves(prec, rec, confidences, output_dir, label):
    ...
    fig_dir = os.path.join(output_dir, f"figs/p_r_curve")
    if not os.path.exists(fig_dir):
        os.makedirs(fig_dir)
...
def plot_multiple_p_r_curves(prec_list, rec_list, confidences_list, max_3d_dists, output_dir, label):

    fig, ax = plot_multiple_p_r(
            prec_list=prec_list,
            rec_list=rec_list,
            confidences_list=confidences_list,
            max_3d_dists=max_3d_dists
        )
