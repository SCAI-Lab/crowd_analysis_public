...
def data_augmentation(sample_dict):
    scans, target_reg = sample_dict["laser_data"], sample_dict["target_reg"]
    ...
    sample_dict.update({"target_reg": target_reg, "laser_data": scans})
...
# ):
#     num_pts = len(scan)
#     target_cls = np.zeros(num_pts, dtype=int64)
#     target_reg = np.zeros((num_pts, 2), dtype=float)
...
def scans_to_cutout(
    ...
    # print("half_alpha shape is: {}".format(half_alpha.shape))

    # cutout indices
    delta_alpha = 2.0 * half_alpha / (num_cutout_pts - 1)
    # print("delta_alpha shape is: {}".format(delta_alpha.shape))
    ang_ct = (
        scan_phi[::stride]
        - half_alpha
    ...
    # print("ang_ct shape is: {}".format(ang_ct.shape))
    inds_ct = (ang_ct - scan_phi[0]) / (scan_phi[1] - scan_phi[0])
    # print("inds_ct shape is: {}".format(inds_ct.shape))
    outbound_mask = np.logical_or(inds_ct < 0, inds_ct > num_pts - 1)

    # cutout (linear interp)
    inds_ct_low = _clip(np.floor(inds_ct), 0, num_pts - 1).astype(int)
    inds_ct_high = _clip(inds_ct_low + 1, 0, num_pts - 1).astype(int)
    ...
    if area_mode:
        ...
        # print("num_pts_in_window is {}".format(num_pts_in_window))
        ...
        if np.sum(area_mask) > 0:
            ...
            inds_ct_area = np.rint(_clip(inds_ct_area, 0, num_pts - 1)).astype(int)
    ...
    return np.ascontiguousarray(
        ct.transpose((2, 1, 0)), dtype=float
    )  # (scans, times, cutouts)
...
def scans_to_cutout_original(
    ...
    scans_cutout = np.empty((num_pts, num_scans, num_cutout_pts), dtype=float)
...
def scans_to_polar_grid(
    ...
    polar_grid = np.empty((num_scans, num_range, num_pts), dtype=float)

    scans = np.clip(scans, min_range, max_range)
    scans_grid_inds = ((scans - min_range) / range_bin_size).astype(int)
    ...
    for i_scan in range(num_scans):
        for i_pt in range(num_pts):
            ...
            if tsdf_clip > 0.0:
                ...
                tsdf = (
                    np.arange(min_dist, max_dist, step=1).astype(float)
                    * range_bin_size
                )
                tsdf = np.clip(tsdf, -tsdf_clip, tsdf_clip)
            else:
                tsdf = np.zeros(num_range, dtype=float)
...
def group_predicted_center(
    ...
    instance_mask = np.zeros(len(scan_grid), dtype=int)
    ...
    grid = np.zeros((x_range, y_range, pred_cls.shape[1]), float)
    ...
    pred_x_inds = int64((pred_xs - x_min) / bin_size)
    pred_y_inds = int64((pred_ys - y_min) / bin_size)
...
def nms_predicted_center(
    scan_grid, phi_grid, pred_cls, pred_reg, pred_box=None, pred_reg_prev=None, min_dist=0.5
):
    ...
    # Global in laser frame
    pred_xs, pred_ys = canonical_to_global_xy(scan_grid, phi_grid, pred_reg[:,0], pred_reg[:,1])
    ...
    instance_mask = np.zeros(num_pts, dtype=int)
    instance_id = 1
    ...
    if pred_box is not None:
        det_box = pred_box[keep]

    if pred_reg_prev is None:
        if pred_box is not None:
            return det_xys, det_cls, det_box, instance_mask
        else:
...
def _phi_to_rotation_matrix(phi):
    ...
    return np.array([[cp, -sp], [sp, cp]], dtype=float)
...
def get_velocity_from_odometry(scan1_xy, odom0, odom1):
    ...
    phi_cross = odom_del[2] * np.array([[0, -1], [1, 0]], dtype=float)
...
def generate_pseudo_labels(
    ...
    return (
        np.array(pl_loc_xy, dtype=float),
        np.array(pl_boxes, dtype=float),
        neg_mask,
    )
