...
def get_label_anno(label_path):
    ...
    # Extract columns 3 to 5 and convert them to float
    numeric_columns = content[:, 9:12].astype(float)

    # Filter out corrupted annotations with negative dimension
    valid_mask = (numeric_columns > 0.0).all(axis=1)
    content = content[valid_mask]
    annotations['name'] = content[:, 0]
    annotations['truncated'] = content[:, 1].astype(float)
    annotations['occluded'] = content[:, 2].astype(int)
    annotations['num_points'] = content[:, 3].astype(float)
    annotations['alpha'] = content[:, 4].astype(float)
    annotations['bbox'] = content[:, 5:9].astype(float)
    # dimensions will convert hwl format to standard lhw(camera) format.
    annotations['dimensions'] = content[:, 9:12].astype(float)[:, [2, 0, 1]]
    annotations['location'] = content[:, 12:15].astype(float)
    annotations['rotation_y'] = content[:, 15].astype(float)
    annotations['score'] = content[:, 16].astype(float)

    

    # keep_mask = annotations["num_points"] >= 10
    # for k, v in annotations.items():
    #     annotations[k] = v[keep_mask]

    return annotations

def get_label_annos(label_folder, image_ids=None, ann_mask_2D_folder=None,):
    ...
    if ann_mask_2D_folder is not None:
        ann_mask_2D_folder = pathlib.Path(ann_mask_2D_folder)
    for idx in image_ids:
        image_idx = get_image_index_str(idx)
        label_filename = label_folder / (image_idx + '.txt')
        anno = get_label_anno(label_filename)
        if ann_mask_2D_folder is not None:
            mask_filename = ann_mask_2D_folder / (image_idx + '.npy')
            mask_2D = np.load(mask_filename)
            
            # Apply the mask to filter the annotations
            filtered_anno = {key: value[mask_2D] for key, value in anno.items()}
            annos.append(filtered_anno)
        else:
            annos.append(anno)
    return annos
