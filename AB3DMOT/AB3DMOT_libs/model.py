...
class AB3DMOT(object):			  	
	...
	def __init__(self, max_age, min_hits, thres, log, metric=['iou_3d'], ID_init=0, thres_upper=float('inf')):
		...
		# config
		self.cat = 'Pedestrian'
		self.ego_com = True #cfg.ego_com 			# ego motion compensation
		self.calib = None #calib
		self.oxts = None #oxts
		self.affi_process = False	# post-processing affinity
		algm = 'hungar'
		# add negative due to it is the cost
		# if metric in ['dist_3d', 'dist_2d', 'm_dis']: thres *= -1	
		thres = [
			t * -1 if m in ['dist_3d', 'dist_2d', 'm_dis'] else t 
			for m, t in zip(metric, thres)
		]
		self.algm, self.metric, self.thres, self.thres_upper, self.max_age, self.min_hits = \
			algm, metric, thres, thres_upper, max_age, min_hits

		primary_metric = metric[0]  # Use the first metric as the reference for min/max similarity
		if primary_metric in ['dist_3d', 'dist_2d', 'm_dis']: 
			self.max_sim, self.min_sim = 0.0, -100.
		elif primary_metric in ['iou_2d', 'iou_3d']:   	   
			self.max_sim, self.min_sim = 1.0, 0.0
		elif primary_metric in ['giou_2d', 'giou_3d']: 	   
	...
	# 			elif cat == 'Bicycle': 		
	# 				algm, metric, thres, min_hits, max_age = 'greedy', 'dist_3d',	6, 3, 2
	# 			else: assert False, 'error'
	...
	# 			elif cat == 'Truck': 		
	# 				algm, metric, thres, min_hits, max_age = 'greedy', 'giou_3d', -0.2, 1, 2
	# 			elif cat == 'Trailer': 		
	# 				algm, metric, thres, min_hits, max_age = 'greedy', 'giou_3d', -0.2, 3, 2
	# 			elif cat == 'Bus': 			
	# 				algm, metric, thres, min_hits, max_age = 'greedy', 'giou_3d', -0.2, 1, 2
	# 			elif cat == 'Motorcycle':	
	# 				algm, metric, thres, min_hits, max_age = 'greedy', 'giou_3d', -0.8, 3, 2
	# 			elif cat == 'Bicycle': 		
	# 				algm, metric, thres, min_hits, max_age = 'greedy', 'giou_3d', -0.6, 3, 2
	# 			else: assert False, 'error'
	# 		elif cfg.det_name == 'deprecated':		
	# 			if cat == 'Car': 			
	...
	def print_param(self):
		...
		# print_log('distance thresholds is %f' % self.thres, log=self.log, display=False)
		# Join metrics as a comma-separated string for readability
		print_log('distance metrics are [%s]' % ', '.join(self.metric), log=self.log, display=False)
		# Join thresholds as a comma-separated string, formatted to two decimal places
		print_log('distance thresholds are [%s]' % ', '.join(f'{t:.2f}' for t in self.thres), log=self.log, display=False)
	...
	def track(self, dets_all, frame, seq_name):
		...
		if 'm_dis' in self.metric:
			trk_innovation_matrix = [trk.compute_innovation_matrix() for trk in self.trackers]
		else:
			trk_innovation_matrix = None
		matched, unmatched_dets, unmatched_trks, cost, affi = \
			data_association(dets, trks, self.metric, self.thres, self.algm, trk_innovation_matrix, threshold_upper=self.thres_upper)
